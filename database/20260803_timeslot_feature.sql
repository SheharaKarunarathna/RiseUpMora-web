...
ALTER TABLE candidates
    ADD COLUMN IF NOT EXISTS pref_1_timeslot INT CHECK (pref_1_timeslot IN (1, 2, 3)),
    ADD COLUMN IF NOT EXISTS pref_2_timeslot INT CHECK (pref_2_timeslot IN (1, 2, 3));
...
CREATE TABLE IF NOT EXISTS timeslot_bookings (
    ...
    slot_number INT CHECK (slot_number IN (1, 2, 3)),
...
SELECT c.id, s.slot_number, 0
FROM companies c
CROSS JOIN (VALUES (1), (2), (3)) AS s(slot_number)
WHERE NOT EXISTS (
